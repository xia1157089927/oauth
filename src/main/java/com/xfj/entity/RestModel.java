...
public class RestModel<T> {
    ...
    public static <T> ResponseEntity<RestModel<T>> error(HttpStatus httpStatus, ErrorStatus resultStatus) {
        return new ResponseEntity<RestModel<T>>(new RestModel<T>(resultStatus.getErrcode(), resultStatus.getErrmsg()),
                                                httpStatus);
    }
    ...
    public static <T> ResponseEntity<RestModel<T>> error(HttpStatus httpStatus, ErrorStatus resultStatus, T errorMsg) {
        return new ResponseEntity<RestModel<T>>(new RestModel<T>(resultStatus.getErrcode(),
                                                                 resultStatus.getErrmsg(),
                                                                 errorMsg),
                                                httpStatus);
    }
